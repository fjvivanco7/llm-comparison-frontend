...
> = {
  1: {
    ...
    description: "Código ilegible o no funcional",
    color: "text-red-500",
  },
  2: {
    label: "Deficiente",
    description: "Funciona pero tiene problemas importantes",
    color: "text-orange-500",
  },
  3: {
    label: "Aceptable",
    description: "Cumple lo básico, hay espacio para mejora",
    color: "text-yellow-500",
  },
  4: {
    label: "Bueno",
    description: "Bien estructurado, pocas mejoras necesarias",
    color: "text-green-500",
  },
  5: {
    label: "Excelente",
    description: "Código ejemplar, sigue mejores prácticas",
...
export default function EvaluateCodePage() {
  ...
  const loadCode = async () => {
    try {
      let currentPage = 1;
      let foundCode: PendingCode | null = null;
      const MAX_PAGES = 10; // Límite para evitar bucles infinitos

      while (!foundCode && currentPage <= MAX_PAGES) {
        const response = await api.get(
          `/evaluation/pending?page=${currentPage}&limit=10`,
        );

        if (response.data.data && response.data.data.length > 0) {
          foundCode = response.data.data.find(
            (c: PendingCode) => c.id === codeId,
          );
        }

        if (!foundCode && response.data.meta?.hasNextPage) {
          currentPage++;
        } else {
          break;
        }
    ...
    <div className="space-y-6">
      ...
      <Card className="border-border/50 bg-card/50 backdrop-blur">
        <CardHeader>
          ...
          <CardDescription>
            Califica cada criterio del 1 al 5. Pasa el cursor sobre las
            estrellas para ver la rúbrica.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="grid gap-4 md:grid-cols-2">
            {CRITERIA.map((criterion) => (
              <div
                key={criterion.key}
                className={`p-4 rounded-lg ${criterion.bgColor}`}
              >
                <div className="mb-2">
                  <Label
                    className={`text-base font-semibold ${criterion.color}`}
                  >
                    {criterion.label}
                  </Label>
        ...
        <CardContent className="pt-6">
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                {Object.values(scores).filter((s) => s > 0).length} de 4
                criterios evaluados
              </p>
            </div>
          ...
          <Button
            ...
            disabled={
              isSubmitting || !Object.values(scores).every((s) => s > 0)
            }
