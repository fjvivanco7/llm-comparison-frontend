...
> = {
  ...
  missing_error_handling: {
    label: "Falta manejo de errores",
    icon: AlertTriangle,
    color: "text-amber-500",
  },
  incomplete_code: {
    label: "Código incompleto",
    icon: FileCode,
    color: "text-gray-500",
  },
  poor_naming: {
    label: "Nombres poco descriptivos",
    icon: FileCode,
    color: "text-blue-500",
  },
  no_comments: {
...
export default function MyEvaluationsPage() {
  ...
  const [selectedEvaluation, setSelectedEvaluation] =
    useState<EvaluationWithCode | null>(null);
  ...
  const loadEvaluations = async (page: number) => {
    try {
      const response = await api.get(
        `/evaluation/my-evaluations?page=${page}&limit=${ITEMS_PER_PAGE}`,
      );
  ...
  const handleOpenModal = async (
    evaluation: EvaluationWithCode,
    index: number,
  ) => {
    // Calcular el índice global tomando en cuenta la paginación
  ...
  const getModelColor = (llmName: string) => {
    ...
    const key =
      llmName.toLowerCase().split("-")[0].split("/")[1] ||
      llmName.toLowerCase().split("-")[0];
  ...
  const getScoreDistributionChart = () => {
    ...
    const avgScores = {
      readability:
        evaluations.reduce((sum, e) => sum + e.readabilityScore, 0) /
        evaluations.length,
      clarity:
        evaluations.reduce((sum, e) => sum + e.clarityScore, 0) /
        evaluations.length,
      structure:
        evaluations.reduce((sum, e) => sum + e.structureScore, 0) /
        evaluations.length,
      documentation:
        evaluations.reduce((sum, e) => sum + e.documentationScore, 0) /
        evaluations.length,
    };
      ...
      series: [
        {
          ...
          data: [
            {
              value: avgScores.readability.toFixed(2),
              itemStyle: { color: "#3b82f6" },
            },
            {
              value: avgScores.clarity.toFixed(2),
              itemStyle: { color: "#22c55e" },
            },
            {
              value: avgScores.structure.toFixed(2),
              itemStyle: { color: "#f97316" },
            },
            {
              value: avgScores.documentation.toFixed(2),
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-muted-foreground mt-2">
          {meta?.total || 0}{" "}
          {meta?.total === 1 ? "código evaluado" : "códigos evaluados"}
        </p>
      </div>
                  ...
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground flex items-center gap-1">
                        ...
                        {format(new Date(evaluation.evaluatedAt), "PPp", {
                          locale: es,
                        })}
                      </p>
                    </div>
          ...
          {selectedEvaluation && (
            <>
              <DialogHeader className="flex-shrink-0 pr-8">
                ...
                <DialogDescription>
                  {format(new Date(selectedEvaluation.evaluatedAt), "PPp", {
                    locale: es,
                  })}
                </DialogDescription>
              </DialogHeader>
              ...
              <div className="flex-1 overflow-y-auto space-y-6 mt-4 pr-2">
                ...
                {selectedEvaluation.problemTags &&
                  selectedEvaluation.problemTags.length > 0 && (
